        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if(res['data'].length == 1){
                    ...
                    $("#minMileage").val(res.maxMileage); 
                    if($("#registrationno").length != 0) {                
                        document.getElementById("registrationno").focus();
                    }
                    // open last job card history modal
                        if(actiontype!='UPDATE'){
                            if(res['lastService'].length > 0){
                                $("#dealerName").text(res['lastService'][0]['CustomerName']);
                                $("#lastServiceType").text(res['lastService'][0]['JobTypeName']);
                                $("#lastSchedule").text(res['lastService'][0]['ScheduleTitle']);
                                $("#lastServiceDate").text(res['lastService'][0]['JobDate']);
                                $("#lastFeedback").text(res['lastService'][0]['Feedback']);
                                $("#myModal").modal('show');
                            }
