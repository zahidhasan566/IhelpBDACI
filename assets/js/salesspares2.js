...
$(document).ready(function () {
    ...
    $('#add').on('click', function (event) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success == 1) {
                    if (spqty > parseInt(res.partstock)) {
                        clearAll();
    ...
    $("#affiliator").on('change',function () {
        console.log("========",affiliator_discount_applied,$(this).val());
        if(affiliator_discount_applied == false && $(this).val() !=='') {
            $("#affiliator_discount").attr('disabled',false);
        } else{
            $("#affiliator_discount").attr('disabled',true);
        }

    });
    $("#affiliator_discount").on('click',function() {
        var affiliateDiscount = 10;
        affiliatorDiscountPrice = 0;
        if(affiliator_discount_applied) {
            $(this).attr('disabled', true);
            return false;
        }
        if(orderdetails.rows().eq(0).row(0).length < 1) {
            alert("Please add spareparts to get the discount");
            return false;            
        }

        affiliator_discount_applied = true;
        $(this).prop('disabled',true);
        
        // ******** To apply Affiliator discount in Datatable

        //var newlyAddedData = [];
        // orderdetails.rows().eq(0).each( function ( index ) {
        //     var row = orderdetails.row( index );        
        //     var data = row.data();
        //     data[dndx] = parseFloat(data[dndx]) + affiliateDiscount;
        //     data[dpndx] =  (parseFloat(data[undx]) * parseFloat(data[qndx])) * (parseFloat(data[dndx])/100);
        //     data[tndx] =  (parseFloat(data[undx]) * parseFloat(data[qndx])) - (parseFloat(data[undx]) * parseFloat(data[qndx])) * (parseFloat(data[dndx])/100);
            
        //     newlyAddedData.push(data);
        // });
        // // redraw data table       
        // orderdetails.clear().draw();
        // orderdetails.rows.add(newlyAddedData);
        // orderdetails.columns.adjust().draw();

        // ********* End To apply Affiliator discount in Datatable

        var totalVlaue = $("#biketable tfoot tr th:nth-child(4)").html().replace('/-','');
        totalVlaue = parseFloat(totalVlaue);
        affiliatorDiscountPrice = (totalVlaue * (affiliateDiscount/100)).toFixed(2);

        $(this).html("Discount Applied: "+affiliatorDiscountPrice).removeClass('btn-danger');
        $("#affiliator_discount_price").html(affiliatorDiscountPrice);
        $("#final_price_with_affiliator_discount").html(totalVlaue - affiliatorDiscountPrice);

        console.log("====",affiliatorDiscountPrice);
    });
});

var affiliator_discount_applied = false;
var affiliatorDiscountPrice = 0;
...
function SavePartsInvoice() {
    ...
    var post = {
        ...
        affiliatorDiscountPrice: affiliatorDiscountPrice,
        data: pros 
    };

    console.log("Final====",affiliatorDiscountPrice);
