$(document).ready(function () {
 ...
 function loadajaxfunction(string,page,searchstring){
    ...
    datasting = string; 
        $.ajax({
            url: base_url + "report/loadfreeservice",
            type: "post",
            ...
            success: function (response) {
               ...
               for(var i=0; i<FreeServiceList.length; i++){
                   ...
                   var string2 = '<td>' + FreeServiceList[i]['ProductCode'] + ' - ' +  FreeServiceList[i]['ProductName'] + '</td>\
                        ...
                        <td>' + FreeServiceList[i]['Customer_Type'] + '</td>\
                        ...
                        <td>' + FreeServiceList[i]['DoneStatus'] + '</td>';
                        if(FreeServiceList[i]['DoneStatus'] == 'Done'){
                            var string2 = string2 + '<td>' + FreeServiceList[i]['TakenServiceSameDealer'] + '</td>';
                        }else{
                            var string2 = string2 + '<td></td>';
                        }
                        if(FreeServiceList[i]['Remark']){
                            var string2 = string2 + '<td>Yes</td>';
                        }else{
                            var string2 = string2 + '<td>No</td>';
                        }
                        var string2 = string2 + '<td>'+FreeServiceList[i]['Remark']+'</td>';
               ...
               $("#a_ExportToExcel").attr("href", base_url + "report/freeserviceexcelexport/?" + datasting + "&excelfilename=" + excelfilename); 
