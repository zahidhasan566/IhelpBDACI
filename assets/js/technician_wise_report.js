...
$("#exportToExcel").on('click',function() {
    var DateFrom        = $("#DateFrom").val();
        ...
        document.location = base_url + "jobcard/technicianWiseLoadReport?"+string + "&searchstring="+ searchstring+"&page=&exportable="+'yes';
...
function loadajaxfunction(string, page, searchstring,exportable=false) {
    ...
    datasting = string;
    $.ajax({
        url: base_url + "jobcard/technicianWiseLoadReport",
        type: "post",
        ...
        success: function (response) {
            ...
            for (j = 0; j < receivedata.length; j++) {
                ...
                for (i = 0; i < keys.length; i++) {
                    var format = (receivedata[j][keys[i]]==null) ? "" : receivedata[j][keys[i]];
