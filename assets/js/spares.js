...
$(document).ready(function () {
            ...
            $( "#dialog-confirm" ).dialog({
                ...
                buttons: {
                    "Yes": function() {
                        ...
                        table.cell(row, tndx).data((parseFloat(qdata)+parseFloat(spqty)) * (parseFloat(udata)+parseFloat(vdata))+"/-" );
                        table.draw();
                    },
                    Cancel: function() {
                        $( this ).dialog( "close" );
    ...
    $(".paidparts").autocomplete({        
        ...
        select: function (event, ui) {
            event.preventDefault();
            $(this).val(ui.item.label);
            $("#unitprice").html(ui.item.unitprice);
...
function calculateTotal() {
    ...
    totalprice += Math.round (((unitprice+vat) * quantity)*100) / 100;        
...
function SaveOrder() {
    ...
    var actionurl = $("#frmspare").attr('action');    
    $.ajax({
        type: 'POST',
        url: actionurl,
        dataType: 'json',        
