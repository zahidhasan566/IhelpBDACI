...
$(document).ready(function () {
    ...
    $( "#ReportOrder" ).submit(function( event ) {
        ...
        var string = "CustomerCode=" + CustomerCode + "&ProductCode=" + ProductCode + "&reporttype=" + reporttype + "&page=" + page;
        var savestring = "CustomerCode=" + CustomerCode + "&ProductCode=" + ProductCode + "&reporttype=" + reporttype;
        //alert(base_url);
        //console.log(string);
...
function loadajaxfunction(string,page,searchstring){
    ...
    datasting = string; 
    $.ajax({
        url: base_url + "report/loadProductStock",
        type: "post",
        ...
        success: function (response) {
           ...
           for(var i=0; i<StockList.length; i++){
               ...
               var string2 = '<td>' + StockList[i]['Invoiceno'] + '</td><td>' + StockList[i]['ProductCode'] + ' - ' +  StockList[i]['ProductName'] + '</td>\
                    <td>' + StockList[i]['ReceiveDate'] + '</td>\
                    <td>' + StockList[i]['ReceivedQnty'] + '</td>\
                    <td>' + StockList[i]['UnitPrice'] + '</td>\
                    <td>' + StockList[i]['ChassisNo'] + '</td>\
                    <td>' + StockList[i]['EngineNo'] + '</td>\
                    <td>' + StockList[i]['Color'] + '</td>\
                    </tr>';
           ...
           $("#a_ExportToExcel").attr("href", base_url + "report/stockexcelexport/?" + datasting + "&excelfilename=" + excelfilename);
