...
function loadajaxfunction(string,page,searchstring){
    ...
    datasting = string; 
    $.ajax({
        url: base_url + "report/loadsupplyratio",
        type: "post",
        ...
        success: function (response) {
           ...
           for(var i=0; i<response.length; i++){
               var sl = i + 1;
               var ratio = parseFloat(response[i]['SupplyRatio']);
                
               var string = '<tr><td>' + sl + '</td>\
                    <td>' + response[i]['DealerCode'] + '</td>\
                    <td class="text-right">' + response[i]['OrderQnty'] + '</td>\
                    <td class="text-right">' + response[i]['BackOrderQnty'] + '</td>\
