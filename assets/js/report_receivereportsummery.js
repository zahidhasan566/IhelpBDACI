...
function loadajaxfunction(string, page, searchstring) {
    ...
    datasting = string;
    $.ajax({
        url: base_url + "logistics/loadreceivereportsummery",
        type: "post",
        ...
        success: function (response) {
            ...
            for (j = 0; j < receivedata.length; j++) {
                totalsend = totalsend + parseInt(receivedata[j]['Units']);
                var string = '';
            ...
            $('#dataloadtable tbody').append('<tr><td colspan="3" class="text-right bold">Total</td><td>'+totalsend+'</td></tr>'); 
